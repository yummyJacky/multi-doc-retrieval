"""
Nvidia NemoRetriever RAG Pipeline with FAISS Indexing (FastScan + Refine)
优化版本：支持FastScan快速检索和Refine精确重排机制
- FastScan: 使用Product Quantization加速检索
- Refine: 两阶段检索，先快速筛选再精确计算
"""
...
class FAISSIndexManager:
    ...
    def __init__(
        ...
        index_type: str = "flat",  # 'flat', 'ivfflat', 'ivfpq', 'ivfpq_fastscan'
        nlist: int = 100,  # IVF聚类中心数量
        m: int = 96,  # PQ子向量数量（必须能整除embedding_dim）
        nbits: int = 8,  # PQ每个子向量的比特数
        use_gpu: bool = False,
        gpu_id: int = 0,
        use_refine: bool = False,  # 是否启用Refine机制
        refine_k: int = 100  # Refine时的候选数量
    ):
        """
        初始化FAISS索引
        ...
        Args:
            ...
            index_type: 索引类型
                ...
                - 'ivfpq': 乘积量化，适合大规模(>1M)
                - 'ivfpq_fastscan': FastScan加速的IVF+PQ索引
            nlist: IVF索引的聚类中心数量
            m: PQ子向量数量（必须能整除embedding_dim）
            nbits: PQ每个子向量的比特数
            use_gpu: 是否使用GPU加速索引
            gpu_id: GPU设备ID
            use_refine: 是否启用Refine机制（两阶段检索）
            refine_k: Refine时的候选数量
        """
        ...
        self.original_embeddings = None  # 用于Refine的原始向量
    ...
    def _create_index(self) -> faiss.Index:
        ...
        if self.index_type == "flat":
            ...
            # IVF + 乘积量化
            quantizer = faiss.IndexFlatIP(self.embedding_dim)
            index = faiss.IndexIVFPQ(quantizer, self.embedding_dim, self.nlist, self.m, self.nbits)
            
        elif self.index_type == "ivfpq_fastscan":
            # IVF + PQ + FastScan
            # FastScan要求m必须是4的倍数，且nbits通常为4或8
            if self.m % 4 != 0:
                raise ValueError(f"FastScan requires m to be a multiple of 4, got {self.m}")
            
            quantizer = faiss.IndexFlatIP(self.embedding_dim)
            # 使用IndexIVFPQFastScan构建 FastScan 索引
            index = faiss.IndexIVFPQFastScan(quantizer, self.embedding_dim, self.nlist, self.m, self.nbits)
            print(f"✓ 使用FastScan索引: m={self.m}, nbits={self.nbits}")
        else:
    ...
    def build_index(
        ...
        # 保存原始向量用于Refine
        if self.use_refine:
            self.original_embeddings = all_token_embeddings.copy()
            print("✓ 已保存原始向量用于Refine机制")
        ...
        # 训练索引（IVF类型需要）
        if self.index_type in ["ivfflat", "ivfpq", "ivfpq_fastscan"]:
    ...
    def search(
        ...
        # 第一阶段：快速筛选候选
        # 如果启用Refine，第一阶段搜索更多候选
        if self.use_refine and self.original_embeddings is not None:
            k_per_token = self.refine_k  # 第一阶段搜索refine_k个候选
        else:
        ...
        for q_idx in range(len(query_embedding)):
            ...
            for nn_idx in range(k_per_token):
                token_idx = indices[q_idx, nn_idx]
        ...
        if self.use_refine and self.original_embeddings is not None:
            ...
            for page_id, _ in candidates:
                for idx, pid in enumerate(self.page_id_map):
                    if pid == page_id:
                        candidate_token_indices.append(idx)
            ...
            for q_idx in range(len(query_embedding)):
                ...
                for idx, global_idx in enumerate(candidate_token_indices):
                    page_id = self.page_id_map[global_idx]
                    # 精确计算内积
                ...
                for page_id, score in token_scores.items():
                    if page_id in refined_page_scores:
                        refined_page_scores[page_id] += score
                    else:
                        refined_page_scores[page_id] = score
            
            # 返回Refine后的结果
            sorted_pages = sorted(refined_page_scores.items(), key=lambda x: x[1], reverse=True)
...
class NvidiaRAGPipeline:
    ...
    def __init__(
        ...
        if use_faiss:
            ...
            self.faiss_manager = FAISSIndexManager(
                ...
                m=faiss_config.get("m", 96),  # PQ子向量数量
                nbits=faiss_config.get("nbits", 8),  # PQ比特数
                use_gpu=faiss_config.get("use_gpu", False),
                gpu_id=device,
                use_refine=faiss_config.get("use_refine", False),  # 是否启用Refine
                refine_k=faiss_config.get("refine_k", 100)  # Refine候选数
            )
        else:
    ...
    def build_unified_index(self, save_dir: Optional[str] = None):
        ...
        # 保存原始向量用于Refine
        if self.faiss_manager.use_refine:
            self.faiss_manager.original_embeddings = self.all_token_embeddings.copy()
            print("\n✓ 已保存原始向量用于Refine机制")
        
        # 构建FAISS索引
        print("\n构建FAISS索引...")
        self.faiss_manager.index = None  # 清空旧索引
        self.faiss_manager.page_id_map = token2pageuid  # 设置映射
        
        # 重新初始化索引
        embedding_dim = self.all_token_embeddings.shape[1]
        quantizer = faiss.IndexFlatIP(embedding_dim)
        
        if self.faiss_manager.index_type == "flat":
            index = quantizer
        ...
        elif self.faiss_manager.index_type == "ivfpq":
            index = faiss.IndexIVFPQ(
                quantizer,
                embedding_dim,
                self.faiss_manager.nlist,
                self.faiss_manager.m,
                self.faiss_manager.nbits
            )
        elif self.faiss_manager.index_type == "ivfpq_fastscan":
            # FastScan索引
            if self.faiss_manager.m % 4 != 0:
                raise ValueError(f"FastScan requires m to be a multiple of 4, got {self.faiss_manager.m}")
            ...
            print(f"  ✓ 使用FastScan索引: m={self.faiss_manager.m}, nbits={self.faiss_manager.nbits}")
        else:
        ...
        if self.faiss_manager.index_type != "flat":
            print("  训练索引...")
            # 归一化用于训练
            train_data = self.all_token_embeddings.copy()
            faiss.normalize_L2(train_data)
            index.train(train_data)
        
        print("  添加向量...")
        # 归一化用于添加
        add_data = self.all_token_embeddings.copy()
