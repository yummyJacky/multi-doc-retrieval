...
class VisDoMRAG:
    ...
    def _build_interactive_visual_index(self):
        if getattr(self, "_visual_index_built", False):
            return
        ...
        page_embeddings = {}
        page_info = {}
        for doc_id, pdf_path in tqdm(pdf_paths.items(), desc="Processing PDFs for interactive visual index"):
            try:
                pages = convert_from_path(pdf_path)
            except Exception as e:
                logger.error(f"Error converting PDF {pdf_path} to images: {str(e)}")
                traceback.print_exc()
                continue
            for page_idx, page_img in enumerate(pages):
                page_id = f"{doc_id}_{page_idx}"
                try:
                    if self.vision_retriever in ["colpali", "colqwen"]:
                        ...
                        with torch.no_grad():
                            embedding = self.vision_model(**processed_image)
                        page_embeddings[page_id] = embedding.cpu()
                        page_info[page_id] = {"doc_id": doc_id, "page_idx": page_idx, "pdf_path": pdf_path}
                except Exception as e:
                    logger.error(f"Error processing page {page_idx} of PDF {pdf_path}: {str(e)}")
                    traceback.print_exc()
                    continue
    ...
    def retrieve_visual_contexts_for_question(self, query):
        try:
            ...
            if not getattr(self, "_visual_index_built", False):
                return []
            if not getattr(self, "_page_embeddings", None):
                return []
            page_ids = list(self._page_embeddings.keys())
            if not page_ids:
                logger.warning("No visual embeddings available for interactive retrieval")
                return []
            try:
                ...
                logger.error(f"Error generating query embedding for interactive visual retrieval: {str(e)}")
                traceback.print_exc()
                return []
            ds = torch.cat([self._page_embeddings[pid] for pid in page_ids], dim=0)
            if len(ds) == 0:
            ...
            for page_id in selected_page_ids:
                ...
                try:
                    ...
                    traceback.print_exc()
                    continue
            logger.info("Retrieved %d visual contexts for interactive query", len(pages))
            return pages
        except Exception as e:
            logger.error(f"Error retrieving visual contexts for interactive query: {str(e)}")
            traceback.print_exc()
            return []

    def _build_interactive_text_index(self):
        if getattr(self, "_text_index_built", False):
            return
        if not self.document_cache:
            self.cache_documents()
        all_chunks = []
    ...
    def retrieve_textual_contexts_for_question(self, question):
        try:
            ...
            if not getattr(self, "_text_index_built", False):
                return []
            contexts = []
            if self.text_retriever == "bm25":
                ...
                return []
            logger.info("Retrieved %d textual contexts for interactive query", len(contexts))
            return contexts
        except Exception as e:
            logger.error(f"Error retrieving textual contexts for interactive query: {str(e)}")
            traceback.print_exc()
            return []
